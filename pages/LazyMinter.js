...
var voucherContract = undefined;
var voucherSigner = undefined;
...
class LazyMinter {
   ...
   * @returns {NFTVoucher}
   */
  async createVoucher(tokenIds, urls, redeemer) {
    const voucher = { tokenIds, urls, redeemer }
    ...
    const signature = await voucherSigner._signTypedData(domain, types, voucher)
    return {
      ...voucher,
      signature,
    }
  ...
  async _signingDomain() {
    ...
    const chainId = await voucherContract.getChainID()
    this._domain = {
      name: SIGNING_DOMAIN_NAME,
      version: SIGNING_DOMAIN_VERSION,
      verifyingContract: voucherContract.address,
      chainId,
    }
    return this._domain
  }
